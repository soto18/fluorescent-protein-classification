...
random_state = pd.read_csv("/home/dmedina/plastic_classs_models/configs/random_states.csv")
random_state = random_state["random_state"].values

df_to_train = pd.read_csv(doc_config.readline().replace("\n", ""))
output_path = doc_config.readline().replace("\n", "")
plastic_type = doc_config.readline().replace("\n", "")
